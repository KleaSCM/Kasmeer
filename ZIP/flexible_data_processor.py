...
class FlexibleDataProcessor:
    # Flexible data processor that works with any company's datasets
    ...
    def __init__(self, config_path: str = "config.yaml"):
        # Initialize the flexible data processor
        # Args:
        ...
        logger.info(f"Initialized FlexibleDataProcessor with config_path={config_path}")
        ...
        if isinstance(infra_data, pd.DataFrame):
            ...
            if 'latitude' in infra_data.columns and 'longitude' in infra_data.columns:
                ...
                return {
                    ...
                    'nearest_infrastructure_type': infra_data.loc[nearest_idx, 'Pipe Type'] if 'Pipe Type' in infra_data.columns else 'unknown',
    ...
    def _extract_vegetation_features(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        if isinstance(veg_data, pd.DataFrame):
            # Handle tabular vegetation data
            zone_types = []
            if 'Type' in veg_data.columns:
                zone_types = veg_data['Type'].unique().tolist()
            ...
            # Spatial intersection for geospatial data
            point = Point(lon, lat)
            intersecting = veg_data[veg_data.geometry.intersects(point)]
            
            zone_types = []
            if 'Type' in intersecting.columns:
                zone_types = intersecting['Type'].unique().tolist()  # type: ignore
