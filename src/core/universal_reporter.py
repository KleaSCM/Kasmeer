...
class UniversalReporter:
        ...
        if coord_cols['lat'] and coord_cols['lon']:
            ...
            if not lat_numeric.isna().all() and not lon_numeric.isna().all():
                overview['geographic_bounds'] = {
                    'lat_min': lat_numeric.min(),
                    'lat_max': lat_numeric.max(),
                    'lon_min': lon_numeric.min(),
                    'lon_max': lon_numeric.max()
                }
    ...
    def _analyze_coordinates(self, dataset: pd.DataFrame, coord_cols: Dict[str, str]) -> Dict[str, Any]:
    ...
    def _analyze_proximity(self, dataset: pd.DataFrame, coord_cols: Dict[str, str], location: Dict) -> Dict[str, Any]:
